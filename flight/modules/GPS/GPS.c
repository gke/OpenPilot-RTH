...
volatile float NavYawCorr = 0.0f;
volatile float NavAltCorr = 0.0f;
float homeLongitudeCorrection = 1.0f;
...
#if defined(INCLUDE_CC_NAV)
	#define EXTRA_BYTES   (\
			sizeof(StabilizationDesiredData)\
			+sizeof(HomeLocationData)\
			+sizeof(AttitudeStateData)\
			+sizeof(AccessoryDesiredData\
	))
#else
...
// Unfortunately need a good size stack for the WMM calculation
		...
		#define STACK_SIZE_BYTES   (580+EXTRA_BYTES) // NMEA
#else
        #define STACK_SIZE_BYTES   (440+EXTRA_BYTES)
#endif // PIOS_INCLUDE_GPS_NMEA_PARSER
#else
        #define STACK_SIZE_BYTES   650 // NOT ADJUSTED FOR INCLUDE_CC_NAV YET
...
int32_t GPSInitialize(void)
{
...
#if defined(REVOLUTION) || defined(INCLUDE_CC_NAV)
    ...
    if (gpsPort && gpsEnabled) {
        ...
        GPSSatellitesInitialize();
#endif
#if defined(PIOS_GPS_SETS_HOMELOCATION) || defined(INCLUDE_CC_NAV)
        HomeLocationInitialize();
#endif
        updateHwSettings();
    }
        ...
        if ((timeNowMs - timeOfLastUpdateMs) >= GPS_TIMEOUT_MS ||
            ...
            if ((gpspositionsensor.PDOP < gpsSettings.MaxPDOP) && (gpspositionsensor.Satellites >= gpsSettings.MinSatellites) &&
                ...
                if (home.Set == HOMELOCATION_SET_TRUE)
                	computeNAVCorrection(&gpspositionsensor);
                else
                	 ZeroNav();
#endif
            } else {
            	if ((gpspositionsensor.Status == GPSPOSITIONSENSOR_STATUS_FIX3D) &&
                       (gpspositionsensor.Latitude != 0 || gpspositionsensor.Longitude != 0)) {
...
{ // derived/crunched from updateFixedDesiredAttitude()
	...
	if ( AccessoryDesiredInstGet(0, &accessory) != 0) // later simple rth sw / stab mode
		...
		float NorthE = home.Latitude - gpsData->Latitude;
		float EastE = (home.Longitude - gpsData->Longitude) * homeLongitudeCorrection;
		...
		if (((GetCurrentFrameType() == FRAME_TYPE_FIXED_WING) || (GetCurrentFrameType() == FRAME_TYPE_CUSTOM))
...
#if defined(NAV_USE_ALT_HOLD)
			// assume increased throttle gives increased AS hence lift when held in level flight
			if (stabDesired.StabilizationMode.Pitch == STABILIZATIONDESIRED_STABILIZATIONMODE_ATTITUDE) {
			// check if RTH is active or not
				TODO:
			} else
				NavAltCorr = 0.0f;
#else
			...
			ZeroNav(); // must be YAW
#else
			if (((GetCurrentFrameType() == FRAME_TYPE_MULTIROTOR) ||
					(GetCurrentFrameType() == FRAME_TYPE_HELI)) &&
			(stabDesired.StabilizationMode.Roll == STABILIZATIONDESIRED_STABILIZATIONMODE_ATTITUDE) &&
...
static void setHomeLocation(GPSPositionSensorData *gpsData)
{
    ...
    home.Longitude = gpsData->Longitude;
    home.Altitude  = gpsData->Altitude + gpsData->GeoidSeparation;
    homeLongitudeCorrection = cos(DEG2RAD(home.Latitude * 1e-7f));

    home.g_e = 9.80655f; // close enough
    home.Set = HOMELOCATION_SET_TRUE;
    HomeLocationSet(&home);
