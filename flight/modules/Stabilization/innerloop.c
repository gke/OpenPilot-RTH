...
extern volatile float NavRollCorr, NavPitchCorr, NavYawCorr;
#endif
...
static void stabilizationInnerloopTask()
{
    ...
    for (t = 0; t < AXES; t++) {
        ...
        if (t < STABILIZATIONSTATUS_INNERLOOP_THRUST) {
            ...
            case STABILIZATIONSTATUS_INNERLOOP_AXISLOCK:
                if (fabsf(rate[t]) > stabSettings.settings.MaxAxisLockRate) {
                    ...
                    axis_lock_accum[t]  = boundsymf(axis_lock_accum[t], stabSettings.settings.MaxAxisLock);
...
#if defined(INCLUDE_CC_NAV) && !defined(NAV_USE_ROLL)
            	rate[t] = boundsymf(rate[t] - (t == 2 ? NavYawCorr : 0.0f),
            	    StabilizationBankMaximumRateToArray(stabSettings.stabBank.MaximumRate)[t]);
#else
            	rate[t] = boundsymf(rate[t],
            		StabilizationBankMaximumRateToArray(stabSettings.stabBank.MaximumRate)[t]);
#endif
            ...
            {
                float stickinput[3];
                stickinput[0] = boundsymf(rate[0] / stabSettings.stabBank.ManualRate.Roll, 1.0f);
                stickinput[1] = boundsymf(rate[1] / stabSettings.stabBank.ManualRate.Pitch, 1.0f);
                stickinput[2] = boundsymf(rate[2] / stabSettings.stabBank.ManualRate.Yaw, 1.0f);
                rate[t] = boundsymf(rate[t],
                    StabilizationBankMaximumRateToArray(stabSettings.stabBank.MaximumRate)[t]);
        ...
        actuatorDesiredAxis[t] = boundsymf(actuatorDesiredAxis[t], 1.0f);
    }
