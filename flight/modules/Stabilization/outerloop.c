...
extern volatile float NavRollCorr, NavPitchCorr, NavYawCorr, NavAltCorr;
extern float cruisecontrol_factor;
#endif
...
static void stabilizationOuterloopTask()
{
    ...
    float local_error[3];
    {
    	...
    	local_error[0] = (stabilizationDesiredAxis[0] - NavRollCorr) - attitudeState.Roll;
    	local_error[1] = (stabilizationDesiredAxis[1] - NavPitchCorr) - attitudeState.Pitch; // for multi
        ...
        local_error[2] =  modulo < 0 ?
        		modulo + 180.0f : modulo - 180.0f;
    ...
    for (t = 0; t < AXES; t++) {
        ...
        if (t < STABILIZATIONSTATUS_OUTERLOOP_THRUST) {
            ...
            {
                float stickinput[3];
                stickinput[0] = boundsymf(stabilizationDesiredAxis[0] / stabSettings.stabBank.RollMax, 1.0f);
                stickinput[1] = boundsymf(stabilizationDesiredAxis[1] / stabSettings.stabBank.PitchMax, 1.0f);
                stickinput[2] = boundsymf(stabilizationDesiredAxis[2] / stabSettings.stabBank.YawMax, 1.0f);
                ...
                rateDesiredAxis[t] = boundsymf(pid_apply(&stabSettings.outerPids[t], local_error[t], dT),
                ...
                // Also does not take dT into account
            {
                ...
                weak_leveling = boundsymf(weak_leveling, stabSettings.settings.MaxWeakLevelingRate);
...
#if defined(INCLUDE_CC_NAV) && defined(NAV_USE_ALT_HOLD)
    if (GetCurrentFrameType() == FRAME_TYPE_FIXED_WING)
    	cruisecontrol_factor = rateDesired.Thrust > 0.1f ?
    	1.0f + NavAltCorr : 1.0f;
    else
...
static void AttitudeStateUpdatedCb(__attribute__((unused)) UAVObjEvent *ev)
{
    ...
    if ((cpusaver++ % OUTERLOOP_SKIPCOUNT) == 0) { // 4
